...
void CommandParser::parser(const int argc, const char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "-h") == 0) {
            ...
            this->inputFilesPaths.emplace_back(pathPrefix + argv[i]);
        }
    }

    if (inputFilesPaths.empty()) {
        throw std::invalid_argument(
                "No input files provided. Correct form: -c <config.txt> -o <output.wav> <input1.wav> [<input2.wav> ...]");
    }

    if (outputFilePath.empty()) {
        throw std::invalid_argument(
                "Output file not specified. Correct form: -c <config.txt> -o <output.wav> <input1.wav> [<input2.wav> ...]");
    }
}
...
std::vector<std::string> CommandParser::getInputFilesPaths() const {
    return this->inputFilesPaths;
}

std::string CommandParser::getOutputFilePath() const {
...
void CommandParser::printHelp() {
    ...
    std::cout
            << "     - Mixes the main audio stream with the additional input stream ($[number] specifies the additional input file)."
            << std::endl;
    std::cout << "     - The mixing starts at the specified start time (in seconds)." << std::endl;
    std::cout << "  3. boost [start] [end] [1-10]" << std::endl;
    std::cout << "     - Boosts the volume by a factor in the range of 1 to 10, within the specified time interval."
              << std::endl;
